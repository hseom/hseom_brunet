...
namespace Brunet.Services.Coordinate {
  ...
  public class NCService : IRpcHandler {
...
#if NC_DEBUG
      if (_cnode != null) {
        //Console.Error.WriteLine("[NCService] {0} EchoVivaldiState() method invoked.", _cnode.Address);
      }
#endif
...
#if NC_DEBUG
      if (_cnode != null) {
        //Console.Error.WriteLine("[NCService] {0} EchoVivaldiState() returning.", _cnode.Address);
      }
#endif  
      return ht;
    }
    ...
    public void ComputePathLatencyTo(AHAddress a, object req_state) {
      ...
      ConnectionTable tab = _cnode.ConnectionTable;
      ConnectionList structs = tab.GetConnections(ConnectionType.Structured);
      Connection next_closest = structs.GetNearestTo((AHAddress) _cnode.Address, a);
      //Okay, we have the next closest:
      ListDictionary my_entry = new ListDictionary();
      my_entry["node"] = _cnode.Address.ToString();
      if( next_closest != null ) {
        ...
        result.CloseEvent += delegate(object o, EventArgs args) { // when CloseEvent is occured, delegate function would be called
          Channel q = (Channel)o;
          if( q.Count > 0 ) {
            try {
              ...
              string m = String.Format("<node>{0}</node> trying <connection>{1}</connection> got <exception>{2}</exception>", _cnode.Address, next_closest, x);
    ...
    public class TrialState {
      ...
      public TrialState(Node n, TrialState.TrialFinishedCallback cb) {
        _node = n;
        _callback = cb;
        ArrayList con_list = new ArrayList(); //connection list
        foreach (Connection con in _node.ConnectionTable.GetConnections(ConnectionType.Structured))
        {
          //if (con.Edge is Tunnel.TunnelEdge || (con.Address != TransAddressCheck(_node.Address))) {
	...
	//if (target.Address != TransAddressCheck(_node.Address)){
	...
	Address q_addr = new AHAddress(target);
      ...
      protected void GetVivaldiState() {
        ...
        q.CloseEvent += delegate(object o, EventArgs args) { //what is the 'closed'? it means that queue is filled with response
      ...
      protected void GetLatencySample() {
        ...
        Channel q = new Channel(1);
        q.CloseEvent += delegate(object o, EventArgs args) { //when CloseEvent is occured, delegate function would be called
    ...
    public VivaldiState State {
      get {
        lock(_sync) {
          ...
          v_state.Position = new Point(_vivaldi_state.Position);
    ...
    protected Node _cnode; //_cnode is main for vivaldi coordinate calculation
    ...
    //in order to invoke cluster algorithm
    protected double _current_x;
    protected double _current_y;
    protected double _updated_x;
    protected double _updated_y;
    protected bool _clusteron;
     ...
     */
    public NCService(Node cnode, Node qnode, Point InitialPoint) {
      _sync = new object();
      _cnode = null;
      _qnode = null;
      ...
      if(cnode != null && qnode != null) {
      //if(node != null) {
        _cnode = cnode;
	_qnode = qnode;

#if NC_DEBUG
        Console.Error.WriteLine("[NCService] {0} Starting an instance of NCService.", _cnode.Address);
#endif 
        lock(_sync) {
          _rpc = _cnode.Rpc;
          _rpc.AddHandler("ncserver", this);
          _cnode.HeartBeatEvent += GetNextSample;
        }
      }
    }
...
*/
    public NCService(Node cnode, Node qnode):
      this(cnode, qnode, (Point) null)
    {
    }
/*
    ...
    */
    ...
    protected void GetNextSample(object node, EventArgs args) {
      ...
      try {
        TrialState new_trial = new TrialState(_cnode, TrialComplete); 
        ...
        */
        ...
        Console.Error.WriteLine("[NCService] {0}, {1}", _cnode.Address, x.Message);
#else 
                         ) {
#endif        
      }
    ...
    protected void TrialComplete(TrialState trial, DateTime instant, Address target_address, 
...
#if NC_DEBUG
      Console.Error.WriteLine("[NCService] {0} Trial complete to: {1}.", _cnode.Address, target_address);
#endif
      if (_current_trial != trial) {
#if NC_DEBUG
        Console.Error.WriteLine("[NCService] {0} Too late now: {1}.", _cnode.Address, target_address);
#endif
        return;
      }
      ...
      Console.Error.WriteLine("[NCService] {0} Trial result at: {1}, from: {2} {3}, position: {4}, error: {5}, latency: {6}", 
                              _cnode.Address, instant.Ticks, host, target_address, position, weighted_error, latency);
#endif
     ...
     */
    public void ProcessSample(DateTime o_stamp, string o_host, Address o_neighbor, Point o_position, double o_weightedError, double o_rawLatency) {
      lock(_sync) {
        Sample sample = null;
        if (_samples.ContainsKey(o_neighbor)) {
...
#if NC_DEBUG
        Console.Error.WriteLine("Sample at: {0}, from: {1} {2}, position: {3}, error: {4}, raw latency: {5}, smooth latency: {6}", o_stamp, o_host, o_neighbor, o_position, o_weightedError, o_rawLatency, o_latency);
#endif
	...
	_current_x = _vivaldi_state.Position.Side[0];
	_current_y = _vivaldi_state.Position.Side[1];
  ...
  _updated_x = _vivaldi_state.Position.Side[0];
  _updated_y = _vivaldi_state.Position.Side[1];
  ...
  Console.Error.WriteLine("Updated position: {0}, distance: {1}", 
     ...
     double variance = Math.Sqrt(Math.Pow((_current_x - _updated_x), 2) + Math.Pow((_current_y - _updated_y), 2)); 
     if(variance < 0.1 && _clusteron == false) {
       _clusteron = true;
       DeetooCluster dc = new DeetooCluster(_cnode, _qnode, _updated_x, _updated_y);
       dc.Start();
     }
    }
